...
func (as *AnimationSender) SendByFileId(fileId string, silent bool) (*objs.SendMethodsResult, error) {
	return as.bot.apiInterface.SendAnimation(
		as.chatIdInt, as.chatidString, fileId, nil, as.caption, as.parseMode,
		as.width, as.height, as.duration, as.replyTo, as.thumb, as.thumbFile,
		silent, as.allowSendingWihoutReply, as.captionEntities, as.replyMarkup,
	)
}

/*Sends a file on the web. The file is downloaded on telegram server and then will be sent to the chat.*/
func (as *AnimationSender) SendByURL(url string, silent bool) (*objs.SendMethodsResult, error) {
	return as.bot.apiInterface.SendAnimation(
...
func (as *AnimationSender) SendByFile(file *os.File, silent bool) (*objs.SendMethodsResult, error) {
	stat, err := file.Stat()
	if err != nil {
		return nil, err
	}
...
func (as *AnimationSender) SetThumbnail(fileIdOrURL string) {
	as.thumb = fileIdOrURL
}

/*This method sets the thumbnail of the file. It takes a file existing on the device*/
func (as *AnimationSender) SetThumbnailFile(file *os.File) error {
	stat, err := file.Stat()
	if err != nil {
		return err
	}
